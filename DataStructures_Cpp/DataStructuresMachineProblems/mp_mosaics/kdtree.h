...
class KDTree
{
  ...
  public:
    ...
    //Helper functions for constructor and destructor;

    void createkd(KDTreeNode* & nod, vector<Point<Dim>> newpts, int dim);

    unsigned findmed(vector<Point<Dim>> & lol, int dim);

    void destkd(KDTreeNode*nod);
    ...
    //helper function for find nearest
    void findnn(KDTreeNode* nod, Point<Dim> &currentBest,Point<Dim> &q,  int dim) const;
