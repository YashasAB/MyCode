...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
   ...
   for(auto it = frequencies.begin() ; it != frequencies.end(); ++it){
     TreeNode * lol = new TreeNode(*it);
     singleQueue.push(lol);
   }

   while(!(mergeQueue.size() == 1 && singleQueue.empty())){
         TreeNode* s;
         TreeNode* ss;

         s = removeSmallest(singleQueue, mergeQueue);
         ss = removeSmallest(singleQueue, mergeQueue);
         int f = s->freq.getFrequency()+ ss->freq.getFrequency();
         TreeNode* curr = new TreeNode(f);
         curr->left = s;
         curr->right = ss;
         mergeQueue.push(curr);
      }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     if(!bfile.getNextBit()){
       ...
       lol = new TreeNode(f);
       lol->left=readTree(bfile);
       lol->right=readTree(bfile);
     }
   // }
     return NULL;
}
